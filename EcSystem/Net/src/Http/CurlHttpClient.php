...
use ES\Net\Http\Exception\CurlHttpException;
...
class CurlHttpClient
{
    ...
    {
        $http_option = config('common.http_options');
    ...
    public function flush()
    {
        ...
        if ($this->error_code || !$this->response || 200 != $http_code) {
            $error = 'CurlHttpClient: Could not connect to ' . $full_url;
            FluentLogger::error('http', [
                'curl_error' => curl_error($this->curl_handle), 'http_code' => $http_code
            ]);
