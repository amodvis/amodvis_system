...
class TopClient
{
	...
	public const HTTPS_URL = "https://eco.taobao.com/router/rest";
...
//		$logger->log($logData);
        ...
        $logger = app(LogIntf::class);
        $logger->setSeparator('^_^');
        $logger->setLogFile(rtrim(TOP_SDK_WORK_DIR,
...
//			));
    ...
    public function getCacheKey($request): string
    {
        ...
        //签名
        $cacheArr = array_merge($apiParams, $sysParams);

        ksort($cacheArr);

        $str = '';
        foreach($cacheArr as $k => $v) {
            $str .= vsprintf('%s%s', [$k, $v]);
        }

        return strtoupper(md5($str));
    }

    protected function checkReqParams($request)
    {
        $result =  new ResultSet();
